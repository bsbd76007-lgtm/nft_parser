...
class FragmentParser:
    ...
    async def parse_fragment(self, slug: str, number: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if True:
                ...
                if result["status"] is None and result["price_ton"] is None:
                    self.debug_stats["blocked_or_empty"] = self.debug_stats.get("blocked_or_empty", 0) + 1
                    lowered = html.lower()
                    challenge_markers = [
                    ...
                    if self.debug_stats["blocked_or_empty"] <= 3:
                        logger.warning(
                            "Fragment page for %s-%s returned 200 but no status/price parsed "
                            "(len=%d, challenge_marker=%s, snippet=%r)",
                            slug, number, len(html), hit, html[:300]
                        )
    ...
    async def search_random_gifts(
        ...
        try:
            ...
            logger.exception(f"Random search error: {e}")
        finally:
            self.active_searches[user_id] = False
    ...
    async def search_by_filter(
        ...
        try:
            ...
            logger.exception(f"Filter search error: {e}")
        finally:
            self.active_searches[user_id] = False
    ...
    async def search_gifts_filtered(
        ...
        try:
            ...
            logger.exception(f"Filtered search error: {e}")
        finally:
            self.active_searches[user_id] = False
